...
async function testPlaceId(placeId, name) {
	...
	try {
		...
		if (data.reviews && data.reviews.length > 0) {
			...
			console.log(`      Autor: ${review.authorAttribution?.displayName || 'Anónimo'}`);
			console.log(`      Rating: ${'⭐'.repeat(review.rating || 0)}`);
			console.log(`      Texto: ${review.text?.text?.substring(0, 100) || 'Sin texto'}...`);
		}
...
async function main() {
	...
	// Test Pamplona
	const pamplonaData = await testPlaceId(PLACE_IDS[0], "CLINICA BORRAS (Pamplona)");
	...
	if (successCount === 2) {
		const totalReviews = results.reduce((sum, r) => sum + (r?.reviews?.length || 0), 0);
